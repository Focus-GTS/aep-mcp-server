 ...
 * three places to edit, run this after `npm version`:
 *
 *   npm version minor        # bumps package.json, commits, tags
 *   npm run sync-version     # updates server.json to match
 *   git commit --amend --no-edit && git tag -f "v$(node -p "require('./package.json').version")"
 *
 * Or simply let the `version` lifecycle script handle it — see package.json,
 * where this runs automatically and stages server.json into the version commit.
 */
