...
const inputSchema = {
  name: z
    ...
    .max(255)
    .describe(
      "Human-readable name for the segment (e.g. 'High Value Customers')",
    ),
  description: z
    .string()
    .max(4096)
  ...
  pqlFormat: z
    ...
    .default("pql/text")
    .describe(
      "Format of the PQL expression: 'pql/text' (human-readable) or 'pql/json' (AST)",
    ),
  schemaName: z
    .string()
    .min(1)
    ...
    .describe(
      "XDM schema class the segment evaluates against (defaults to _xdm.context.profile)",
    ),
  evaluationType: z
  ...
  ttlInDays: z
    ...
    .optional()
    .describe(
      "Optional time-to-live in days for profile membership in this segment",
    ),
};
