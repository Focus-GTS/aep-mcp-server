...
const inputSchema = {
  ...
  name: z
    .string()
    .optional()
    .describe("Optional friendly name for the query (helps identify it in query history)"),
  description: z
    .string()
    .optional()
...
export function register(server: McpServer, ctx: ToolContext): void {
  server.tool(TOOL_NAME, TOOL_DESCRIPTION, inputSchema, async (args) => {
    ...
    try {
      logger.debug(
        { tool: TOOL_NAME, name, dbName: dbName ?? DEFAULT_DB_NAME, sqlLength: sql.length },
        "Submitting query",
      );
